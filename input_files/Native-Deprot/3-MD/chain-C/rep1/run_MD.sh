...
#
#SBATCH --job-name=Init4_9JGM     # The name of your job, you'll se it in squeue.
...
$grom grompp -f run_MD.mdp -c $Dir/../02_initial/init4.gro -r $Dir/../02_initial/init4.gro -p $Dir/../01_box-min/${sys}.top -n $ndx -o run_MD.tpr -nice 19 -maxwarn 100 >> ${sys}.out 2>> ${sys}.err
mpirun -np $SLURM_NTASKS $grom mdrun -v -deffnm run_MD -pin on -bonded gpu -pme gpu -nb gpu -ntomp $SLURM_CPUS_PER_TASK -npme 1 -tunepme -dlb auto -maxh 23.9 >> run_MD.out 2>> run_MD.err
