...
sed -i '/ATOM   2249  P /d' ${pdbid}_fixed_${chain}.pdb
sed -i '/ATOM   2250  OP1 /d' ${pdbid}_fixed_${chain}.pdb
sed -i '/ATOM   2251  OP2 /d' ${pdbid}_fixed_${chain}.pdb
sed -i 's/MG    MG A/MG    MG B/g' ${pdbid}_fixed_${chain}.pdb
...
# Manually reorder and adjust the residue numbering by hand for the MGs and the OH3
...
sed -i 's/MG/mMg/g' ${sys}_Ion_chain_B.itp

# Small bug. The first include is not in the right position.
# Not CpHMD so not correction is needed
