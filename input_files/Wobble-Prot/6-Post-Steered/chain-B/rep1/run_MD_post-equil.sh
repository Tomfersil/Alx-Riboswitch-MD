...
#
#SBATCH --job-name=Wobble_9JGM     # The name of your job, you'll se it in squeue.
...
#
#SBATCH --gres=gpu:4                         # Total number of GPUs for the job (MAX: 2 x number of nodes, only available on gpu1 and gpu2)
#SBATCH --gpus-per-node=4            # Number of GPUs per node (MAX: 2, only available on gpu1 and gpu2)
...
$grom grompp -f run_MD_post-equil.mdp -c $Dir/../../06_eRMSD/rep${rep}/equil/run_equil_ermsd.gro -r $Dir/../../06_eRMSD/rep${rep}/equil/run_equil_ermsd.gro -p $Dir/../../01_box-min/${sys}.top -n $ndx -o run_MD_equil.tpr -nice 19 -maxwarn 100 >> ${sys}.out 2>> ${sys}.err
mpirun -np $SLURM_NTASKS $grom mdrun -v -deffnm run_MD_equil -pin on -bonded gpu -pme gpu -nb gpu -ntomp $SLURM_CPUS_PER_TASK -npme 1 -tunepme -dlb auto -maxh 23.9 >> run_MD_equil.out 2>> run_MD_equil.err
