...
#
#SBATCH --job-name=11.00_9JGM-D-P     # The name of your job, you'll se it in squeue.
...
# Info about the Job:
echo "Job executed in Machine $HOSTNAME" > $SLURM_SUBMIT_DIR/9JGM-D-P_${j}.blockinfo
echo "Job executed with $nCPU processors" >> $SLURM_SUBMIT_DIR/9JGM-D-P_${j}.blockinfo


# Copy important files for local directory; first gro is called 
mkdir -p /leonardo_scratch/large/userexternal/tfernand/Projects/Alx-ribo/W-Prot/chain-D/09_CpH-restr/wobble/rep1/pH11.00/tfernand_CpHMD$$
 
echo "Job executed in DIR: /leonardo_scratch/large/userexternal/tfernand/Projects/Alx-ribo/W-Prot/chain-D/09_CpH-restr/wobble/rep1/pH11.00/tfernand_CpHMD$$ " >> $SLURM_SUBMIT_DIR/9JGM-D-P_${j}.blockinfo

echo "" >> $SLURM_SUBMIT_DIR/9JGM-D-P_${j}.blockinfo
echo -e "Job started on: " >> $SLURM_SUBMIT_DIR/9JGM-D-P_${j}.blockinfo
date >> $SLURM_SUBMIT_DIR/9JGM-D-P_${j}.blockinfo
# Enter local directory
cd /leonardo_scratch/large/userexternal/tfernand/Projects/Alx-ribo/W-Prot/chain-D/09_CpH-restr/wobble/rep1/pH11.00/tfernand_CpHMD$$
...
if [ ${i} -eq 1 ]; then 
   echo $1 
   rsync -r $SLURM_SUBMIT_DIR/$1 9JGM-D-P_${j}.pHmdp
elif [[ ${plumedtype} == "grid" ]] ;then
   rsync -r $SLURM_SUBMIT_DIR/9JGM-D-P_${l}.gro ./
   #rsync -r $SLURM_SUBMIT_DIR/HILLS_${j} ./
   rsync -r $SLURM_SUBMIT_DIR/colvar ./
   rsync -r $SLURM_SUBMIT_DIR/GRIDS ./
   sed "s/GROin=.*/GROin=9JGM-D-P_${l}.gro/g" $SLURM_SUBMIT_DIR/$1 > 9JGM-D-P_${j}.pHmdp
...
Seg_tot_time=`awk -v n=10 '$2 ~ /^EffectiveSteps=/ {m=substr ($2,16); print (n*1000)/(m*0.002)}' 9JGM-D-P_${j}.pHmdp`

sed -i "s/InitCycle=.*/InitCycle=$(($Seg_tot_time*$k+1))/g" 9JGM-D-P_${j}.pHmdp
sed -i "s/EndCycle=.*/EndCycle=$(($Seg_tot_time*$i))/g"  9JGM-D-P_${j}.pHmdp

# line needed to mount /programs using autofs
#ls /programs/ >/dev/null 2>&1; sleep 2
#
CpHMDDIR=`egrep "CpHDIR="  9JGM-D-P_${j}.pHmdp 2>/dev/null| awk '{print substr($2,9,length($2)-9)}' 2>/dev/null`
...
awk '/#mdp# / {print}' 9JGM-D-P_${j}.pHmdp | sed 's/#mdp# //g' > 9JGM-D-P.mdp

## Decompose the fixgro options from the run_CpHMD into an individual fixgro ##
awk '/#fixgro# / {print}' 9JGM-D-P_${j}.pHmdp | sed 's/#fixgro# //g' > 9JGM-D-P.fixgro
#
## Decompose the plumed options from the run_CpHMD into an individual plumed.dat ##
if [[ $plumedtype == "grid" ]] ;then
    awk '/#plumed# / {print}' 9JGM-D-P_${j}.pHmdp | sed 's/#plumed# //g' > 9JGM-D-P_plumed.dat
    sed -i "s/\&colvar_stride/1000/g" 9JGM-D-P_plumed.dat
    sed -i "s/\&grid_name/GRIDS/g" 9JGM-D-P_plumed.dat
    sed -i "s/\&hills/HILLS/g" 9JGM-D-P_plumed.dat
    sed -i "s/\&colvar_name/colvar/g" 9JGM-D-P_plumed.dat
fi
...
#nice -n 19 /tmp/tfernand_CpHMD$$/CpHMD.sh  9JGM-D-P_${j}.pHmdp > 9JGM-D-P_${j}.err 2>&1 
nice -n 19 bash /leonardo_scratch/large/userexternal/tfernand/Projects/Alx-ribo/W-Prot/chain-D/09_CpH-restr/wobble/rep1/pH11.00/tfernand_CpHMD$$/CpHMD.sh 9JGM-D-P_${j}.pHmdp > 9JGM-D-P_${j}.err 2>&1 

# Copy files and return to shared directory
rsync -r /leonardo_scratch/large/userexternal/tfernand/Projects/Alx-ribo/W-Prot/chain-D/09_CpH-restr/wobble/rep1/pH11.00/tfernand_CpHMD$$/9JGM-D-P_${j}* $SLURM_SUBMIT_DIR
if [ ! -f $SLURM_SUBMIT_DIR/9JGM-D-P.sites ] ; then
rsync ./9JGM-D-P.sites $SLURM_SUBMIT_DIR/
fi
if [ -f /leonardo_scratch/large/userexternal/tfernand/Projects/Alx-ribo/W-Prot/chain-D/09_CpH-restr/wobble/rep1/pH11.00/tfernand_CpHMD$$/colvar ] && [ -f /leonardo_scratch/large/userexternal/tfernand/Projects/Alx-ribo/W-Prot/chain-D/09_CpH-restr/wobble/rep1/pH11.00/tfernand_CpHMD$$/HILLS ] && [[ $plumedtype != "grid" ]]  ; then
rsync -r ./colvar $SLURM_SUBMIT_DIR
rsync -r ./HILLS $SLURM_SUBMIT_DIR
fi
#
if [ -f /leonardo_scratch/large/userexternal/tfernand/Projects/Alx-ribo/W-Prot/chain-D/09_CpH-restr/wobble/rep1/pH11.00/tfernand_CpHMD$$/colvar ] && [[ $plumedtype == "grid" ]]  ; then
rsync -r ./colvar $SLURM_SUBMIT_DIR
...
if [ -f /leonardo_scratch/large/userexternal/tfernand/Projects/Alx-ribo/W-Prot/chain-D/09_CpH-restr/wobble/rep1/pH11.00/tfernand_CpHMD$$/colvar ] && [[ $plumedtype == "static" ]]  ; then
    if [ -f /leonardo_scratch/large/userexternal/tfernand/Projects/Alx-ribo/W-Prot/chain-D/09_CpH-restr/wobble/rep1/pH11.00/colvar ] ; then    
...
#
if [ -f /leonardo_scratch/large/userexternal/tfernand/Projects/Alx-ribo/W-Prot/chain-D/09_CpH-restr/wobble/rep1/pH11.00/tfernand_CpHMD$$/colvar ] && [[ $plumedtype == "generic" ]]  ; then
rsync -r ./colvar $SLURM_SUBMIT_DIR
fi

if (for f in 9JGM-D-P_${j}*; do diff $f $SLURM_SUBMIT_DIR/$f; done); then
gzip -9 9JGM-D-P_${j}.{err,log,tpr}
cd $SLURM_SUBMIT_DIR
rm -rf /leonardo_scratch/large/userexternal/tfernand/Projects/Alx-ribo/W-Prot/chain-D/09_CpH-restr/wobble/rep1/pH11.00/tfernand_CpHMD$$
gzip -9 9JGM-D-P_${j}.{err,log,tpr}
else
echo "Error in file copy... please check local files" >> $SLURM_SUBMIT_DIR/9JGM-D-P_${j}.blockinfo
exit 1
fi
#
echo "" >> $SLURM_SUBMIT_DIR/9JGM-D-P_${j}.blockinfo
echo -e "Job finished on: " >> $SLURM_SUBMIT_DIR/9JGM-D-P_${j}.blockinfo
date >> $SLURM_SUBMIT_DIR/9JGM-D-P_${j}.blockinfo
#
