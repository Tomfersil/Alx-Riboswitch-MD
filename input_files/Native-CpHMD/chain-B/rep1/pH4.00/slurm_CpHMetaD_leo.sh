...
#SBATCH --cpus-per-task=16  #$nCPU       # Number of threads per MPI rank (MAX: 2x32 cores on _partition_2, 2x20 cores on _partition_1)
...
if [ -f ${simdir}/${USER}_CpHMD\$$/${colvar_name} ] && [ -f ${simdir}/${USER}_CpHMD\$$/${hills} ] && [[ \$plumedtype != "grid" ]]  ; then
rsync -r ./${colvar_name} \$SLURM_SUBMIT_DIR
rsync -r ./${hills} \$SLURM_SUBMIT_DIR
fi
#
if [ -f ${simdir}/${USER}_CpHMD\$$/${colvar_name} ] && [[ \$plumedtype == "grid" ]]  ; then
rsync -r ./${colvar_name} \$SLURM_SUBMIT_DIR
mv ${hills}_curr_seg ${hills}_\${j}
rsync -r ./${hills}_\${j} \$SLURM_SUBMIT_DIR
rsync -r ./${grid_name} \$SLURM_SUBMIT_DIR 
fi

if [ -f ${simdir}/${USER}_CpHMD\$$/${colvar_name} ] && [[ \$plumedtype == "static" ]]  ; then
    if [ -f ${rundir}/${colvar_name} ] ; then    
        rsync -r ./${colvar_name} \$SLURM_SUBMIT_DIR/${colvar_name}_\${j}
        cat ${colvar_name}_\${j} ${colvar_name} > tmp_${colvar_name}
        mv tmp_${colvar_name} ${colvar_name}
    else
        rsync -r ./${colvar_name} \$SLURM_SUBMIT_DIR/${colvar_name}_\${j}
        rsync -r ./${colvar_name} \$SLURM_SUBMIT_DIR/${colvar_name}
    fi
fi
#
if [ -f ${simdir}/${USER}_CpHMD\$$/${colvar_name} ] && [[ \$plumedtype == "generic" ]]  ; then
rsync -r ./${colvar_name} \$SLURM_SUBMIT_DIR
fi
...
sbatch -A Sis25_bussi $SysName.sh $1
