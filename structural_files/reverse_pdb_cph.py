...
def parse_pdb_frames(file_path):
    """Parse the PDB file and return a list of frames, each containing atoms."""
    frames = []
    atoms = []
    current_model = None
    ...
    # Handle single-frame PDBs (no MODEL/ENDMDL)
    if atoms:
        frames.append(atoms)
...
# Parse frames
frames = parse_pdb_frames(input_file)

# Multiple frames → split output
if len(frames) > 1:
    ...
    for i, atoms in enumerate(frames, start=1):
        ...
        frame_output = f"{base}_{i}.{ext}"
        write_pdb(corrected_atoms, frame_output)
        print(f"Frame {i} written to {frame_output}")
